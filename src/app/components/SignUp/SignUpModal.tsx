...
export default function SignUpModal({ isOpen, onClose, onOpenSignIn }: SignUpModalProps) {
  ...
  const createUserDocument = async (uid: string, email: string, authMethod: 'email' | 'google') => {
    try {
  ...
  // Complete email link sign up when the link is opened
  useEffect(() => {
    const run = async () => {
      ...
      await createUserDocument(cred.user.uid, cred.user.email || stored, 'email');
      onClose();
    };
    run();
  }, [onClose]);
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      await createUserDocument(result.user.uid, result.user.email || '', 'google');
      onClose();
    } catch (err) {
